...
function escape_html() {
  console.log('test');
  const code_block = document.getElementById('escape_html');
  if (code_block) {
    const raw_component = code_block.innerHTML;
    console.log(raw_component);
    let escaped_component = raw_component.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#039;');
    console.log(escaped_component);
    code_block.innerHTML = escaped_component;
  }
}
